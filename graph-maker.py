...
st.markdown("**Beautiful • Professional • Smart unit handling**")
...
with col1:
    ...
    x_sug = st.selectbox("Common X options", x_common, index=2)
...
with col2:
    ...
    y_sug = st.selectbox("Common Y options", y_common, index=2)
...
with tab2:
    ...
    edited = st.data_editor(st.session_state.data, num_rows="dynamic", use_container_width=True, hide_index=True)
        ...
        try:
            ...
            if "Line" in graph_choice:
                ...
                if not is_numeric_y:
                    ...
                    st.stop()
                # Best Histogram - Maximum Quality
                fig = px.histogram(
                    df, 
                    x=y_label, 
                    nbins=25,
...
st.caption("💡 Tip: Change units → Click 'Refresh Data' → Generate Graph again. Histogram is now much better!")
